...
find_all_project_by_current_user_id: Callable[[Session, int], list[ProjectModel]] = (
    lambda the_data, owner_id: the_data.query(ProjectModel)
    .filter(ProjectModel.owner_id == owner_id)
    .all()
)

find_owner_project_by_id: Callable[[Session, int], ProjectMemberModel | None] = (
    lambda the_data, user_id: the_data.query(ProjectMemberModel)
    .filter(ProjectMemberModel.user_id == user_id, ProjectMemberModel.role == "OWNER")
    .first()
)
...
def post_project(db: Session, project_in: ProjectInput, current: UserModel):
    ...
    the_project_duplicate_name = (
        db.query(ProjectModel).filter(ProjectModel.name == project_in.name).first()
    )
    if the_project_duplicate_name is not None:
        return "THE NAME PROJECT IS DUPLICATE !"
    check_number_of_project = find_all_project_by_current_user_id(db, current.id)
    if len(check_number_of_project) >=5:
        return "NOT OVER 5 PROJECT !"
    try:
...
def get_projects(
    ...
    if role:
        the_projetct = the_projetct.filter(ProjectMemberModel.role == role)
    if keyword:
...
def get_detail_project(db: Session, current_user: UserModel, project_id: int):
    ...
    member_projetct = (
        db.query(ProjectMemberModel)
        .join(ProjectMemberModel.project)
        .options(joinedload(ProjectMemberModel.project))
        .filter(ProjectModel.id == the_project.id)
        .all()
    )
...
def path_project(
    db: Session, id: int, current_user: UserModel, update_project: ProjectUpdate
):
    ...
    the_project_duplicate_name = (
        ...
        .filter(ProjectModel.name == update_project.name, ProjectModel.id != id)
        .first()
    )
...
def delete_project(db: Session, id: int, current_user: UserModel):
    ...
    the_members = find_list_of_member_in_the_project(db, id)
    if len(the_members) >= 3:
        return "NOT HAVE DELETE THE PROJECT !"
    the_project.is_delete = True
    member_in_project = (
        db.query(ProjectMemberModel)
        .filter(ProjectMemberModel.project_id == the_project.id)
        .all()
    )
    for m in member_in_project:
        m.is_delete = True
    task_in_that_project = db.query(TaskModel).filter(
        TaskModel.project_id == the_project.id
    )
